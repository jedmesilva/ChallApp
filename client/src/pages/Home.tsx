...
const MissionsApp = () => {
  ...
  const Toast = ({ message, show }: { message: any; show: boolean }) => (
    <div className={`fixed top-20 left-1/2 transform -translate-x-1/2 z-50 transition-all duration-500 ${
      ...
      <div className={`px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 border glass-card dark:glass-card-dark ${
        message.type === 'success' ? 'bg-orange-50/80 dark:bg-orange-500/20 border-orange-200/50 dark:border-orange-400/30 text-orange-800 dark:text-orange-300' :
        message.type === 'info' ? 'bg-blue-50/80 dark:bg-blue-500/20 border-blue-200/50 dark:border-blue-400/30 text-blue-800 dark:text-blue-300' :
        'glass text-foreground border-border'
      }`}>
  ...
  const FiltersModal = () => (
    <div className={`fixed inset-0 z-50 transition-all duration-300 ${showFilters ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      ...
      <div className={`absolute bottom-0 left-0 right-0 bg-card dark:bg-card rounded-t-3xl p-6 transition-transform duration-300 ${showFilters ? 'translate-y-0' : 'translate-y-full'}`}>
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-4">
            <button 
              ...
              className="flex-1 bg-secondary hover:bg-secondary/80 text-secondary-foreground py-3 rounded-2xl font-semibold transition-colors"
            >
              Cancelar
            </button>
            <button 
              onClick={() => setShowFilters(false)}
              className="flex-1 bg-primary hover:bg-primary/90 text-primary-foreground py-3 rounded-2xl font-semibold transition-colors"
            >
              Aplicar Filtros
            </button>
          </div>
        ...
        <div className="bg-gradient-to-r from-orange-500 to-red-500 p-6 pt-12 text-white">
          <div className="flex items-center gap-4 mb-4">
            <button 
              ...
              className="p-2 glass-card dark:glass-card-dark hover:bg-white/30 dark:hover:bg-black/30 rounded-full transition-colors"
            >
        ...
        <div className="p-4 space-y-4 -mt-4">
          {acceptedMissions.length === 0 ? (
            <div className="text-center py-16">
              <div className="w-20 h-20 bg-gradient-to-br from-orange-100 to-red-100 dark:from-orange-500/20 dark:to-red-500/20 rounded-full flex items-center justify-center mx-auto mb-4">
                <Heart className="w-8 h-8 text-orange-500" />
              </div>
              <h3 className="text-lg font-semibold text-foreground mb-2">Nenhuma missão ainda</h3>
              <p className="text-muted-foreground text-sm">Aceite missões para começar a trabalhar</p>
            </div>
          ) : (
            acceptedMissions.map((acceptedMission, index) => (
              <div key={acceptedMission.id} className="bg-card dark:bg-card rounded-3xl p-5 shadow-sm border border-border hover:shadow-md transition-shadow">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="font-bold text-card-foreground text-lg">{acceptedMission.title}</h3>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-muted-foreground text-sm">{acceptedMission.location}</span>
                    </div>
